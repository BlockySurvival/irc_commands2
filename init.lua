...
minetest.chat_send_player = function(name, message) -- luacheck: ignore
	for nick, loggedInAs in pairs(irc2_users) do
		if name == loggedInAs and not minetest.get_player_by_name(name) then
			irc2:say(nick, message)
		end
	end
	return old_chat_send_player(name, message)
end

irc2.register_hook("NickChange", function(user, newNick)
	for nick, player in pairs(irc2_users) do
		if nick == user.nick then
			irc2_users[newNick] = irc2_users[user.nick]
			irc2_users[user.nick] = nil
		end
	end
end)

irc2.register_hook("OnPart", function(user, channel, reason)
	irc2_users[user.nick] = nil
end)

irc2.register_hook("OnKick", function(user, channel, target, reason)
	irc2_users[target] = nil
end)

irc2.register_hook("OnQuit", function(user, reason)
	irc2_users[user.nick] = nil
end)

irc2.register_bot_command("login", {
	...
	func = function(user, args)
		...
		local channel = irc2.conn.channels[irc2.config.channel]
		if not channel then
		...
		if auth and minetest.check_password_entry(playerName, auth.password, password) then
			...
			irc2_users[user.nick] = playerName
...
irc2.register_bot_command("logout", {
	description = "Logout",
	func = function (user, args)
		if irc2_users[user.nick] then
			minetest.log("action", user.nick.."@IRC logs out from "
				..irc2_users[user.nick])
			irc2_users[user.nick] = nil
...
irc2.register_bot_command("cmd", {
	...
	func = function (user, args)
		...
		if not irc2_users[user.nick] then
			return false, "You are not logged in."
		end
		...
		if not minetest.check_player_privs(irc2_users[user.nick], command.privs) then
			return false, "Your privileges are insufficient."
		end
		minetest.log("action", user.nick.."@IRC runs "
			..args.." as "..irc2_users[user.nick])
		return command.func(irc2_users[user.nick], (params or ""))
	end
})

irc2.register_bot_command("say", {
	params = "message",
	...
	func = function (user, args)
		...
		if not minetest.check_player_privs(irc2_users[user.nick], {shout=true}) then
			minetest.log("action", ("%s@IRC tried to say %q as %s"
				.." without the shout privilege.")
					:format(user.nick, args, irc2_users[user.nick]))
			return false, "You can not shout."
		end
		minetest.log("action", ("%s@IRC says %q as %s.")
				:format(user.nick, args, irc2_users[user.nick]))
		minetest.chat_send_all("<"..irc2_users[user.nick].."@IRC> "..args)
